...
class TestPublishMail:
    ...
    @pytest.mark.asyncio
    async def test_publish_register_mail(
        self, mock_channel: MagicMock, mock_exchange: MagicMock
    ) -> None:
        ...
        with (
            patch("app.rabbitmq.publisher.get_channel", return_value=mock_channel),
            patch("app.rabbitmq.publisher.setup_topology", return_value=(mock_exchange, {})),
        ):
            # Publish mail
    ...
    @pytest.mark.asyncio
    async def test_publish_marketing_mail(
        self, mock_channel: MagicMock, mock_exchange: MagicMock
    ) -> None:
    ...
    @pytest.mark.asyncio
    async def test_publish_message_body_contains_email(
        self, mock_channel: MagicMock, mock_exchange: MagicMock
    ) -> None:
...
class TestPublishTestMessages:
    ...
    @pytest.mark.asyncio
    async def test_publish_test_messages_count(
        self, mock_channel: MagicMock, mock_exchange: MagicMock
    ) -> None:
